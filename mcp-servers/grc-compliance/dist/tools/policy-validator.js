...
export async function validatePolicyCompliance(params) {
    ...
    if (missingElements.length > 0) {
        recommendations.push(`Add the following missing elements: ${missingElements.slice(0, 5).join(', ')}${missingElements.length > 5 ? ` and ${missingElements.length - 5} more` : ''}`);
    }
    const partiallyMet = findings.filter((f) => f.status === 'partially_met');
    if (partiallyMet.length > 0) {
        recommendations.push(`Enhance coverage for: ${partiallyMet
            .slice(0, 3)
            .map((f) => f.requirement)
            .join(', ')}`);
    }
